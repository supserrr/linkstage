...
class MockChatUserFirestoreWriter extends Mock implements ChatUserFirestoreWriter {}
  ...
  group('ChatUserRemoteDataSource', () {
    group('ensureChatUser', () {
      test('calls writer setChatUser with id, displayName, photoUrl', () async {
        when(() => mockWriter.setChatUser(any(), any())).thenAnswer((_) async {});
    ...
    group('ensureChatUserById', () {
      ...
      test('calls writer setChatUser with id only when userId provided', () async {
        when(() => mockWriter.setChatUser(any(), any())).thenAnswer((_) async {});

        await datasource.ensureChatUserById('user1');

        verify(() => mockWriter.setChatUser('user1', {'id': 'user1'})).called(1);
      });
    });
  });
}
