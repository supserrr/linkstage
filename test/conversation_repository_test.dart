...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockUserRepository.canSendMessageTo(any(), any()))
  ...
  group('ConversationRepository', () {
    group('getOrCreateOneToOneChat', () {
      test('returns chatId from datasource', () async {
        when(() => mockDataSource.getOrCreateOneToOneChat(
              any(),
              any(),
            )).thenAnswer((_) async => 'chat-123');

        final result = await repository.getOrCreateOneToOneChat('current', 'other');

        expect(result, 'chat-123');
        verify(() => mockUserRepository.canSendMessageTo('current', 'other')).called(1);
        verify(() => mockDataSource.getOrCreateOneToOneChat('current', 'other')).called(1);
      });
    });
    ...
    group('sendMessage', () {
      test('calls datasource sendMessage', () async {
        ...
        verify(() => mockDataSource.sendMessage('chat-1', 'sender-1', 'Hello')).called(1);
      });
    });

    group('getOtherParticipant', () {
      test('returns other participant from datasource', () async {
        when(() => mockDataSource.getOtherParticipant(any(), any()))
            .thenAnswer((_) async => (otherUserId: 'other-1', displayName: 'Alice'));

        final result = await repository.getOtherParticipant('chat-1', 'current');

        expect(result, isNotNull);
        expect(result!.otherUserId, 'other-1');
        expect(result.displayName, 'Alice');
        verify(() => mockDataSource.getOtherParticipant('chat-1', 'current')).called(1);
      });
