  ...
  group('AuthBloc', () {
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        ...
        when(
          () => mock.sendSignInLinkToEmail(any()),
        ).thenAnswer((_) async => {});
        return AuthBloc(
      ...
      act: (bloc) =>
          bloc.add(const AuthSendSignInLinkRequested(email: 'test@test.com')),
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      act: (bloc) =>
          bloc.add(const AuthSendSignInLinkRequested(email: 'bad-email')),
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        final mock = MockAuthRepository();
        when(() => mock.signInWithGoogle()).thenThrow(Exception('cancelled'));
        return AuthBloc(
          sendSignInLink: SendSignInLinkUseCase(mock),
          signInWithEmailLink: SignInWithEmailLinkUseCase(mock),
          signInWithGoogle: SignInWithGoogleUseCase(mock),
