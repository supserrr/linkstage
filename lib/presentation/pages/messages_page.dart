...
class _MessagesPageState extends State<MessagesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) => MessagesPageCubit(),
      child: BlocBuilder<MessagesPageCubit, MessagesPageState>(
        builder: (context, ui) {
          final colorScheme = Theme.of(context).colorScheme;
          final pageCubit = context.read<MessagesPageCubit>();
          return Scaffold(
            appBar: AppBar(
                ...
                Padding(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: ui.searchQuery.isNotEmpty
                          ? IconButton(
                              icon: const Icon(Icons.clear),
                              onPressed: () {
                                _searchController.clear();
                                pageCubit.setSearchQuery('');
                              },
                            )
                          : null,
                      filled: true,
                    ...
                    onChanged: (v) => pageCubit.setSearchQuery(v.trim()),
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      AppFilterChip(
                        label: 'All',
                        selected: ui.filter == MessagesChatFilter.all,
                        onTap: () =>
                            pageCubit.setFilter(MessagesChatFilter.all),
                      ),
                      const SizedBox(width: 8),
                      AppFilterChip(
                        label: 'Unread',
                        selected: ui.filter == MessagesChatFilter.unread,
                        onTap: () =>
                            pageCubit.setFilter(MessagesChatFilter.unread),
                      ),
                      const SizedBox(width: 8),
                      AppFilterChip(
                        label: 'Favorites',
                        selected: ui.filter == MessagesChatFilter.favorites,
                        onTap: () =>
                            pageCubit.setFilter(MessagesChatFilter.favorites),
                      ),
                    ],
                  ),
                ),
                Expanded(
                  child: CustomMaterialIndicator(
                    ...
                    child: StreamBuilder<List<ConversationEntity>>(
                      key: ValueKey<int>(ui.refreshNonce),
                      stream: stream,
                      builder: (context, snapshot) {
                        ...
                        var list = snapshot.data!
                            .where((c) => _matchesSearch(c, ui.searchQuery))
                            .toList();
                        if (ui.filter == MessagesChatFilter.unread) {
                          list = list.where((c) => c.unreadCount > 0).toList();
                        }
                        list.sort((a, b) {
                          final at =
                              b.lastMessageAt ?? b.createdAt ?? DateTime(0);
                          final bt =
                              a.lastMessageAt ?? a.createdAt ?? DateTime(0);
                          return at.compareTo(bt);
                        });
                        if (list.isEmpty) {
                          return ui.searchQuery.isNotEmpty
