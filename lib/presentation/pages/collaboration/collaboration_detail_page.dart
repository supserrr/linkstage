...
import '../../bloc/collaboration_detail/collaboration_detail_ui_cubit.dart';
import '../../bloc/collaboration_detail/collaboration_detail_ui_state.dart';
...
class _CollaborationDetailPageState extends State<CollaborationDetailPage> {
  late final CollaborationDetailUiCubit _uiCubit;

  @override
  void initState() {
    super.initState();
    _uiCubit = CollaborationDetailUiCubit(widget.collaboration);
  }
  ...
  Future<void> _confirmCompletionByCreative() async {
    ...
    try {
      ...
      if (mounted) {
        cubit.setConfirmingCompletion(false);
  ...
  Widget build(BuildContext context) {
    return BlocProvider<CollaborationDetailUiCubit>.value(
      value: _uiCubit,
      child: BlocBuilder<CollaborationDetailUiCubit, CollaborationDetailUiState>(
        builder: (context, ui) {
          final theme = Theme.of(context);
          final colorScheme = theme.colorScheme;
          final status = ui.overrideStatus ?? widget.collaboration.status;
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (widget.viewerIsCreative && isPending) ...[
                    ...
                    const SizedBox(height: 24),
                    BlocProvider(
                      create: (_) => _AcceptDeclineBusyCubit(),
                      child: _AcceptDeclineButtons(
                    ...
                    Row(
                      children: [
                        ...
                        if (isCompleted &&
                            widget.viewerIsCreative &&
                            (ui.overrideCreativeConfirmedAt ??
                                    widget.collaboration.creativeConfirmedAt) ==
                                null) ...[
                          const SizedBox(width: 12),
                          Expanded(
                            child: OutlinedButton.icon(
                              onPressed: ui.isConfirmingCompletion
                                  ? null
                              ...
                              icon: ui.isConfirmingCompletion
                                  ? SizedBox(
                                      width: 20,
                                      height: 20,
                                      child:
...
class _AcceptDeclineBusyCubit extends Cubit<bool> {
  _AcceptDeclineBusyCubit() : super(false);

  void setBusy(bool v) => emit(v);
}
...
class _AcceptDeclineButtons extends StatelessWidget {
  ...
  Future<void> _accept(BuildContext context) async {
    final busy = context.read<_AcceptDeclineBusyCubit>();
    ...
    try {
      ...
      sl<PushNotificationService>().notifyUser(
        targetUserId: collaboration.requesterId,
      ...
      context.go(AppRoutes.chatWithUser(collaboration.requesterId));
    } catch (e) {
      if (context.mounted) {
        busy.setBusy(false);
        showToast(context, 'Failed: $e', isError: true);
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<_AcceptDeclineBusyCubit, bool>(
      builder: (context, isBusy) {
        return Row(
          children: [
            Expanded(
              child: OutlinedButton(
                onPressed: isBusy ? null : () => _decline(context),
            ...
            Expanded(
              child: FilledButton(
                onPressed: isBusy ? null : () => _accept(context),
