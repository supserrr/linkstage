...
import '../../bloc/send_collaboration_form/send_collaboration_form_cubit.dart';
import '../../bloc/send_collaboration_form/send_collaboration_form_state.dart';
...
class _SendCollaborationPageState extends State<SendCollaborationPage> {
  ...
  late final SendCollaborationFormCubit _formCubit;
  ...
  void initState() {
    super.initState();
    _formCubit = SendCollaborationFormCubit();
  }
  ...
  Future<void> _submit() async {
    if (_formCubit.state.isSubmitting) return;
    if (!(_formKey.currentState?.validate() ?? false)) return;

    final user = sl<AuthRedirectNotifier>().user;
    if (user == null) return;

    _formCubit.setSubmitting(true);

    final budgetVal = double.tryParse(_budgetController.text.trim());
    final locationVal = _locationController.text.trim().isEmpty
        ? null
        : _locationController.text.trim();
    final fs = _formCubit.state;
    final eventTypeVal =
        (fs.eventType != null &&
            fs.eventType!.isNotEmpty &&
            fs.eventType != 'Other')
        ? fs.eventType
        : null;
    final startTimeVal = fs.startTime.isNotEmpty ? fs.startTime : null;
    final endTimeVal = fs.endTime.isNotEmpty ? fs.endTime : null;
    ...
    try {
      ...
      if (!mounted) return;
      _formCubit.setSubmitting(false);
      showToast(
        context,
  ...
  Widget build(BuildContext context) {
    return BlocProvider<SendCollaborationFormCubit>.value(
      value: _formCubit,
      child: BlocBuilder<SendCollaborationFormCubit, SendCollaborationFormState>(
                        ...
                        child: Column(
                          ...
                          children: [
                            DropdownButtonFormField<String>(
                              initialValue: formState.eventType ?? 'Other',
                              decoration: const InputDecoration(
                              ...
                              onChanged: (v) => context
                                  .read<SendCollaborationFormCubit>()
                                  .setEventType(v),
                            ),
                              ...
                              children: [
                                Expanded(
                                  child: InkWell(
                                    onTap: () async {
                                      final initial =
                                          _parseTime(formState.startTime) ??
                                    ...
                                    child: InputDecorator(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(
                                          color: formState.startTime.isNotEmpty
                                              ? Theme.of(
                                                  context,
                                                ).colorScheme.onSurface
                                              : Theme.of(
                                  ...
                                  child: InkWell(
                                    onTap: () async {
                                      ...
                                      if (picked != null && context.mounted) {
                                        context
                                            .read<SendCollaborationFormCubit>()
                                            .setEndTime(_timeToStorage(picked));
                                      }
                                    },
                                    borderRadius: AppBorders.borderRadius,
                                    child: InputDecorator(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(
                                          color: formState.endTime.isNotEmpty
                                              ? Theme.of(
                                                  context,
                                                ).colorScheme.onSurface
                                              : Theme.of(
