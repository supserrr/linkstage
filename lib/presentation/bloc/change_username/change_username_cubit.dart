...
class ChangeUsernameCubit extends Cubit<ChangeUsernameState> {
  ...
  Future<void> checkAvailability(String username) async {
    ...
    emit(state.copyWith(isCheckingAvailability: true, validationError: null));
  ...
  void clearValidation() {
    emit(state.copyWith(validationError: null, isAvailable: null));
  }
  ...
  static String? _nextChangeDate(DateTime? lastChange) {
    ...
    final next = lastChange.add(
      const Duration(days: ChangeUsernameUseCase.cooldownDays),
    );
