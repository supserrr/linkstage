...
class CreativeDashboardCubit extends Cubit<CreativeDashboardState> {
  ...
  Future<void> toggleSavedCreative(String creativeUserId) async {
    ...
    final savedIds = await _savedCreativesRepository
        ...
        .first;
    final savedProfiles = await _savedCreativesRepository.getSavedProfiles(
      _userId,
    );
    emit(
      state.copyWith(savedCreativeIds: savedIds, savedCreatives: savedProfiles),
    );
  }
  ...
  Future<void> _migrateFollowedPlannersFromPrefs() async {
    final list = _prefs.getStringList(
      AppConstants.creativeFollowedPlannerIdsKey,
    );
  ...
  Future<void> load() async {
    ...
    try {
      ...
      final openEventsFuture = _eventRepository.fetchOpenEvents(limit: 20);
      final followedPlannerIdsFuture = _followedPlannersRepository
          .watchFollowedPlannerIds(_userId)
          .first;
      final pendingBookingsFuture = _bookingRepository
          .getPendingBookingsByCreativeId(_userId);
      final acceptedBookingsFuture = _bookingRepository
          .getAcceptedBookingsByCreativeId(_userId);
      final invitedBookingsFuture = _bookingRepository
          .getInvitedBookingsByCreativeId(_userId);
      final pendingCollaborationsFuture = _collaborationRepository
          .getCollaborationsByTargetUserId(
            _userId,
            status: CollaborationStatus.pending,
          );
      final completedBookingsFuture = _bookingRepository
          .getCompletedBookingsByCreativeId(_userId);
      final savedCreativeIdsFuture = _savedCreativesRepository
          .watchSavedCreativeIds(_userId)
          .first;
      final savedCreativesFuture = _savedCreativesRepository.getSavedProfiles(
        _userId,
      );
      final fellowCreativesFuture = _profileRepository
          ...
          .first
          .then(
            (all) => all.where((p) => p.userId != _userId).take(20).toList(),
          )
      ...
      final pendingCollaborations = results[5] as List<CollaborationEntity>;
      ...
      final acceptedEventIds = acceptedBookings.map((b) => b.eventId).toSet();
      ...
      final countsFuture = idsForCounts.isEmpty
          ? Future<Map<String, int>>.value({})
          : _bookingRepository.getPendingBookingsCountByEventIds(idsForCounts);
      final savedEventsExtraFuture = idsForSavedEvents.isEmpty
      ...
      final appliedEventIds = pendingBookings.map((b) => b.eventId).toSet();
      final notApplied = openEvents
